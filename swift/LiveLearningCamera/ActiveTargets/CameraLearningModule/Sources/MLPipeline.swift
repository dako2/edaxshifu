...
@MainActor
public class MLPipeline: ObservableObject {
    ...
    private func extractThumbnail(from ciImage: CIImage, boundingBox: CGRect, padding: CGFloat = 10) -> Data? {
        return ImageCropUtility.shared.extractThumbnail(from: ciImage, boundingBox: boundingBox, padding: padding)
    }
    
    // MARK: - Main Processing Entry Point
    // Match OptimizedDetectionPipeline signature for compatibility
    public func process(frame ciImage: CIImage) async -> ProcessingResult {
    ...
    public func process(_ ciImage: CIImage) async -> ProcessingResult {
    ...
    private func stabilizeResults(detections: (yoloDetections: [Detection], handDetections: [HandTrackingResult]), 
        ...
        if let tracker = objectTracker {
            ...
            for detection in topDetections {
                let persistentID = objectIDTracker.getOrCreateID(for: detection)
                // Generate thumbnail from the frame
                let thumbnail = extractThumbnail(from: frame, boundingBox: detection.boundingBox)
    ...
    public func exportAnalytics() -> MLAnalyticsReport {
...
public struct ProcessingResult {
    public let trackedObjects: [MemoryTrackedObject]
    public let handDetections: [HandTrackingResult]
    public let sceneContext: AnalyzedScene?
...
public struct MLAnalyticsReport {
